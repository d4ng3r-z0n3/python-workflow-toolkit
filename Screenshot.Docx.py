...
class UpdateChecker:
    def __init__(self, app_instance):
        ...
        self.error_report_endpoint = f"{self.update_server}/api/error-report"
        self.last_error = None

    def log_debug(self, message):
        """Print debug messages to terminal"""
        print(f"[UPDATE] {datetime.now().strftime('%H:%M:%S')} - {message}")
    ...
    def save_error_log(self):
        try:
            ...
            error_log_file = os.path.join(script_dir, 'update_errors.json')
            error_log = []
            
            self.log_debug(f"Saving error log to: {error_log_file}")
            
            if os.path.exists(error_log_file):
                with open(error_log_file, 'r') as f:
                    error_log = json.load(f)
                    self.log_debug(f"Loaded {len(error_log)} existing errors")
            
            if self.last_error:
                error_entry = {
                    'error': self.last_error,
                    'user_email': self.app.settings.get('email', ''),
                    'system_info': self.get_system_info(),
                    'app_version': self.current_version,
                    'session_id': datetime.now().strftime("%Y%m%d_%H%M%S")
                }
                error_log.append(error_entry)
                self.log_debug(f"Added new error: {self.last_error['type']}")
            ...
            self.log_debug(f"Error log saved successfully with {len(error_log)} entries")
                
        except Exception as e:
            self.log_debug(f"Failed to save error log: {e}")
    ...
    def load_error_log(self):
        try:
            ...
            if os.path.exists(error_log_file):
                with open(error_log_file, 'r') as f:
                    errors = json.load(f)
                    self.log_debug(f"Loaded {len(errors)} stored errors for upload")
                    return errors
            return []
        except Exception as e:
            self.log_debug(f"Failed to load error log: {e}")
            return []
    ...
    def clear_error_log(self):
        try:
            ...
            if os.path.exists(error_log_file):
                ...
                self.log_debug("Error log cleared after successful upload")
        except Exception as e:
            self.log_debug(f"Failed to clear error log: {e}")

    def check_for_updates(self, silent=False):
        if not self.app.settings.get('auto_updates', True) and silent:
            return
            
        check_type = "automatic" if silent else "manual"
        self.log_debug(f"Starting {check_type} update check")
            
        def update_check_thread():
            try:
                system_info = self.get_system_info()
                
                # Start with minimal data like curl command
                check_data = {
                    'current_version': self.current_version
                }
                
                # Add additional data only for manual checks to debug
                if not silent:
                    check_data.update({
                    ...
                    if self.last_error:
                        check_data['previous_error'] = {
                            'error_type': self.last_error.get('type'),
                            'error_message': self.last_error.get('message'),
                            'error_timestamp': self.last_error.get('timestamp'),
                            'retry_attempt': True
                        }
                        self.log_debug("Including previous error in manual check")
                
                self.log_debug(f"Sending payload size: {len(str(check_data))} characters")
                self.log_debug(f"Request data: {json.dumps(check_data, indent=2)}")
                self.log_debug(f"Connecting to update server: {self.update_endpoint}")
                
                headers = {
                    'Content-Type': 'application/json',
                    'User-Agent': f'ScreenshotApp/{self.current_version}'
                }
                ...
                if response.status_code == 200:
                    self.log_debug("Update check successful")
                    self.last_error = None
                    
                    stored_errors = self.load_error_log()
                    if stored_errors:
                        self.log_debug(f"Uploading {len(stored_errors)} stored errors")
                        try:
                            error_report = {
                                'error_batch': stored_errors,
                                'timestamp': datetime.now().isoformat()
                            }
                            ...
                            if error_response.status_code == 200:
                                self.log_debug("Error batch uploaded successfully")
                                self.clear_error_log()
                            else:
                                self.log_debug(f"Error upload failed: {error_response.status_code}")
                        except Exception as e:
                            self.log_debug(f"Failed to upload error batch: {e}")
                    
                    try:
                        update_info = response.json()
                        self.log_debug(f"Server response: {json.dumps(update_info, indent=2)}")
                    except:
                        update_info = {}
                        
                    if update_info.get('update_available', False):
                        self.log_debug(f"Update available: {update_info.get('version', 'unknown')}")
                        self.app.root.after(0, lambda: self.prompt_update(update_info))
                    else:
                        self.log_debug("Application is up to date")
                        if not silent:
                            self.app.root.after(0, lambda: messagebox.showinfo("Updates", "You have the latest version!"))
                else:
                    self.log_debug(f"HTTP error: {response.status_code} - {response.reason}")
                    try:
                        error_body = response.text
                        self.log_debug(f"Error response body: {error_body}")
                    except:
                        pass
                    ...
                    if not silent:
                        self.app.root.after(0, lambda: messagebox.showinfo("Update Check", 
                            "Unable to connect to the update server at this time.\n\n" +
                            "This may be due to:\n" +
                            "• Temporary server maintenance\n" +
                            "• Network connectivity issues\n" +
                            "• Firewall or security software restrictions\n\n" +
                            "Please try again in a few minutes. The application " +
                            "continues to function normally without updates.\n\n" +
                            "If this persists, you can disable automatic updates " +
                            "in the License menu."))
                ...
                if not silent:
                    self.app.root.after(0, lambda: messagebox.showinfo("Update Check", 
                        ...
                        "• Firewall allows application connections\n" +
                        "• Network DNS resolution is working\n\n" +
                        "Please try again later. The application works " +
                        "normally without updates."))
                        
            except requests.exceptions.Timeout as e:
                self.log_debug(f"Timeout error: {e}")
                self.last_error = {
                    'type': 'timeout_error',
                    'message': str(e),
                    'timestamp': datetime.now().isoformat()
                ...
                if not silent:
                    self.app.root.after(0, lambda: messagebox.showinfo("Update Check", 
                        ...
                        "Please try again later or disable automatic updates " +
                        "in the License menu if preferred."))
                        
            except Exception as e:
                self.log_debug(f"General error: {type(e).__name__}: {e}")
                import traceback
    ...
    def prompt_update(self, update_info):
        ...
        def download_and_restart():
            try:
                if '?version=' in download_url:
                    version_param = download_url.split('?version=')[1]
                else:
                    version_param = new_version
                    
                full_download_url = f"{self.update_server}/api/download?version={version_param}"
                
                self.log_debug(f"Initiating download from: {full_download_url}")
                ...
                auth_data = {'current_version': self.current_version}
                ...
                self.log_debug(f"Auth response status: {auth_response.status_code}")
                
                response = session.get(full_download_url, headers=headers, stream=True, timeout=60)
                
                self.log_debug(f"Download response status: {response.status_code}")
                self.log_debug(f"Download response headers: {dict(response.headers)}")
                ...
                if response.status_code == 200:
                    ...
                    if 'text/html' in content_type:
                        ...
                        self.log_debug(f"Got HTML instead of file: {error_preview}")
                        messagebox.showerror("Download Error", "Server returned error page instead of update file.")
                        return
                    
                    script_dir = os.path.dirname(os.path.abspath(__file__))
                    new_filename = f"Screenshot.Docx.v{new_version}.py"
                    new_filepath = os.path.join(script_dir, new_filename)
                    
                    total_size = int(response.headers.get('content-length', 0))
                    self.log_debug(f"Downloading file, expected size: {total_size} bytes")
                    ...
                    self.log_debug(f"Download completed: {new_filepath} ({actual_size} bytes)")
                    
                    if actual_size < 10000:
                        with open(new_filepath, 'r', encoding='utf-8', errors='ignore') as f:
                            content_preview = f.read(200)
                            if '<html>' in content_preview.lower() or 'forbidden' in content_preview.lower():
                                os.remove(new_filepath)
                                messagebox.showerror("Download Error", "Downloaded file appears to be an error page.")
                                return
                    
                    with open(new_filepath, 'r', encoding='utf-8', errors='ignore') as f:
                        first_line = f.readline()
                        if not (first_line.startswith('#!/usr/bin/env python') or first_line.startswith('#') or 'import' in first_line):
                            self.log_debug(f"File doesn't look like Python: {first_line[:50]}")
                            messagebox.showerror("Download Error", "Downloaded file doesn't appear to be a Python script.")
                            return
                    
                    installer_path = os.path.join(script_dir, 'install')
                    if not os.path.exists(installer_path):
                        installer_path = os.path.join(script_dir, 'install.py')
                    
                    if os.path.exists(installer_path):
                        restart_command = [sys.executable, installer_path, new_filename]
                        self.log_debug(f"Restart command: {' '.join(restart_command)}")
                        ...
                        messagebox.showinfo("Update Complete", 
                            ...
                            f"The application will restart with the installer.")
                        
                        dialog.destroy()
                        self.app.root.after(100, lambda: self.app.root.quit())
                        ...
                        messagebox.showinfo("Update Complete", 
                            ...
                            f"File: {new_filename}\n" +
                            f"Size: {actual_size:,} bytes\n\n" +
                            f"Run: python install {new_filename}")
                        
                        dialog.destroy()
                        self.app.root.after(100, lambda: self.app.root.quit())
                    
                else:
                    self.log_debug(f"Download failed with status: {response.status_code}")
                    try:
                        error_body = response.text[:300]
                        self.log_debug(f"Error response: {error_body}")
                    except:
                        pass
                        
                    messagebox.showerror("Download Error", 
                        f"Download failed: HTTP {response.status_code}\n\n" +
                        f"Try again later or contact support.")
                    
            except Exception as e:
                self.log_debug(f"Download exception: {e}")
        ...
        def download_only():
            try:
                ...
                session = requests.Session()
                auth_data = {'current_version': self.current_version}
                session.post(self.update_endpoint, json=auth_data, headers={'Content-Type': 'application/json'}, timeout=10)
                
                response = session.get(full_download_url, headers=headers, stream=True, timeout=60)
                
                if response.status_code == 200:
                    script_dir = os.path.dirname(os.path.abspath(__file__))
                    filename = f"Screenshot.Docx.v{new_version}.py"
                    ...
                    messagebox.showinfo("Download Complete", 
                        f"Update downloaded: {filename}\n" +
                        f"Size: {file_size:,} bytes\n\n" +
                        f"Run: python install {filename}")
                    
                    dialog.destroy()
                else:
                    messagebox.showerror("Download Error", f"Download failed: HTTP {response.status_code}")
                    
            except Exception as e:
                self.log_debug(f"Download failed: {e}")
                messagebox.showerror("Download Error", f"Download failed: {str(e)}")
        
        ttk.Button(button_frame, text="Update & Restart", command=download_and_restart).pack(side='right', padx=(5, 0))
        ttk.Button(button_frame, text="Download Only", command=download_only).pack(side='right', padx=(5, 0))
